...
if __name__ ==  "__main__":
    ...
    try:
        ...
        # 获取通讯录的位置
        time.sleep(.5)
        pic = ImageGrab.grab((weChatWindow[0], weChatWindow[1], weChatWindow[0] + 300, weChatWindow[1] + 300))
        pic.save('./image/weixinTongxunluBtn.png')
        tongxunluPosition = app.get_screenxy_from_bmp(u'./image/weixinTongxunluBtn.png', u'./image/tongxunluIcon.png')
        if tongxunluPosition is False:
            DebugLogger('未点击通讯录元素未找到', log_path)
            tongxunluPosition = app.get_screenxy_from_bmp(u'./image/weixinTongxunluBtn.png', u'./image/tongxunluSelectedIcon.png')
        ...
        if tongxunluPosition is not False:
            ...
            app.app.window(title=wechatWindowTitle, class_name=wechatWindowClassName).click_input(
                coords=(tongxunluPosition[0] + 20, tongxunluPosition[1] + 10))

        # 获取<新的联系人>的位置
        pic = ImageGrab.grab((weChatWindow[0], weChatWindow[1], weChatWindow[0]+ 400, weChatWindow[1] + 500))
        pic.save('./image/newCustomerBtn.png')
        newCustomerPosition = app.get_screenxy_from_bmp(u'./image/newCustomerBtn.png', u'./image/newCustomerIcon.png')
        if newCustomerPosition is False:
            newCustomerPosition = app.get_screenxy_from_bmp(u'./image/newCustomerBtn.png', u'./image/newCustomerSelectedIcon.png')
        if newCustomerPosition is not False:
            # 点击新的联系人
            DebugLogger('新的客户元素找到！', log_path)
            app.app.window(title=wechatWindowTitle, class_name=wechatWindowClassName).click_input(
                coords=(newCustomerPosition[0] + 20, newCustomerPosition[1] + 10))
            print('1:'+str((newCustomerPosition[0] + 20, newCustomerPosition[1] + 10)))
            pic = ImageGrab.grab((weChatWindow[0], weChatWindow[1], weChatWindow[2], weChatWindow[1] + 200))
            pic.save('./image/newCustomerBtn.png')
            addCustomerPosition = app.get_screenxy_from_bmp(u'./image/newCustomerBtn.png', u'./image/addCustomer.png')
            if addCustomerPosition is not False:
                DebugLogger('添加客户Icon元素找到！', log_path)
                app.app.window(title=wechatWindowTitle, class_name=wechatWindowClassName).click_input(
                    coords=(addCustomerPosition[0] + 10, addCustomerPosition[1] + 10))
                print('2:'+str((addCustomerPosition[0] + 10, addCustomerPosition[1] + 10)))
                # win32gui.SetForegroundWindow(win32gui.FindWindow(addCustomerWindowClassName, addCustomerWindowTitle))
                addCustomerWindow = win32gui.FindWindow(addCustomerWindowClassName, addCustomerWindowTitle)
                if addCustomerWindow > 0:
                    customerWindow = win32gui.GetWindowRect(addCustomerWindow);
                    DebugLogger('添加客户Icon元素找到3333！', log_path)
                    # 获取<添加>按钮的位置并点击
                    pic = ImageGrab.grab((customerWindow[0], customerWindow[1], customerWindow[2], customerWindow[3]))
                    pic.save('./image/addCustomerDialog.png')
                    addCustomerPosition = app.get_screenxy_from_bmp(u'./image/addCustomerDialog.png', u'./image/addCustomerIcon.png')
                    ...
                    for mobileV, authenticMessage in customInfoDic.items():
                        app.app.window(title=addCustomerWindowTitle, class_name=addCustomerWindowClassName).double_click(
                            coords=(addCustomerPosition[0] + 50, addCustomerPosition[1] + 10))
                        print('3:'+str((addCustomerPosition[0] + 50, addCustomerPosition[1] + 10)))
                        ...
                        if noCustomerHandler > 0:
                            ...
                            app.app.window(title=addCustomerWindowTitle, class_name=addCustomerWindowClassName).double_click(
                                coords=(addCustomerPosition[0] + 50, addCustomerPosition[1] + 10))
                            print('4:'+str((addCustomerPosition[0] + 50, addCustomerPosition[1] + 10)))
                            k.press_key(k.delete_key)
                            ...
                            for i in range(2):
                                ImageGrab.grab(customerWindow).save('./image/addCustomerDialogSubmitIcon.png')
                                addIconPosition = app.get_screenxy_from_bmp(u'./image/addCustomerDialogSubmitIcon.png', u'./image/addCustomIcon.png')
                                if addIconPosition is False:
                                    addIconPosition = app.get_screenxy_from_bmp(
                                        u'./image/addCustomerDialogSubmitIcon.png', u'./image/addCustomIcon2.png')
                                if addIconPosition is not False:
                                    DebugLogger('添加确认按钮找到！', log_path)
                                    app.app.window(title=addCustomerWindowTitle, class_name=addCustomerWindowClassName).click_input(
                                        coords=(addIconPosition[0] + 20, addIconPosition[1] + 10))
                                    print('5:'+str((addIconPosition[0] + 20, addIconPosition[1] + 10)))
                                    sleep(1)
                                    authenticWindow = win32gui.GetWindowRect(win32gui.FindWindow(authenticMessageClassName, authenticMessageConfirmTitle))
                                    pic = ImageGrab.grab(authenticWindow).save('./image/authenticMessageDialog.png')
                                    deleteIconPosition = app.get_screenxy_from_bmp(u'./image/authenticMessageDialog.png', u'./image/deleteAuthenticMessageIcon.png')
                                    app.app.window(title=authenticMessageConfirmTitle, class_name=authenticMessageClassName).click_input(
                                        coords=(deleteIconPosition[0] + 10, deleteIconPosition[1] + 10))
                                    print('6:'+str((deleteIconPosition[0] + 20, deleteIconPosition[1] + 10)))
                                    app.app.window(title=authenticMessageConfirmTitle, class_name=authenticMessageClassName).click_input(
                                        coords=(deleteIconPosition[0] - 30, deleteIconPosition[1] + 10))
                                    print('7:'+str((deleteIconPosition[0] - 30, deleteIconPosition[1] + 10)))
                                    app.inputText(authenticMessage)
                                    sendIconPosition = app.get_screenxy_from_bmp(u'./image/authenticMessageDialog.png',
                                                                                   u'./image/sendAuthenticBtn.png')
                                    if sendIconPosition is False:
                                        sendIconPosition = app.get_screenxy_from_bmp(u'./image/authenticMessageDialog.png',
                                                                                   u'./image/sendAuthenticBtn2.png')
                                    app.app.window(title=authenticMessageConfirmTitle, class_name=authenticMessageClassName).click_input(
                                        coords=(sendIconPosition[0] , sendIconPosition[1] + 10))
                                    print('8:'+str((sendIconPosition[0], sendIconPosition[1] + 10)))
                                    resultTupl = (mobileV, authenticMessage, '插入成功')
                                    resultList.append(resultTupl)
                                    sleep(3)
                                else:
                                    DebugLogger('添加确认按钮未找到！', log_path)
                else:
                    DebugLogger('添加客户窗口未找到！', log_path)
            else:
                DebugLogger('添加客户Icon元素未找到！', log_path)
            # 后期准备（将不必要的dialog都删除）
            authenticMessageWindow = win32gui.FindWindow(authenticMessageClassName, authenticMessageConfirmTitle)
            if authenticMessageWindow > 0:
                app.close(authenticMessageConfirmTitle)
            ...
            DebugLogger('新的客户元素未找到', log_path)
    except Exception as e:
        DebugLogger('导入数据失败！失败原因：%s' % str(e), log_path)
